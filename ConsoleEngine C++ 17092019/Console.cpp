...
void UpdateData(bool noColorChange)
{
	...
	if (noColorChange == true)
		GetConsoleScreenBufferInfoEx(ConsoleOut, &csbiInfo);
	...
	SetConsoleScreenBufferInfoEx(ConsoleOut, &csbiInfo);
}
...
void Console::Create(short ScreenWidth, short ScreenHeight)
{
	...
	SetConsoleMode(ConsoleIn, ENABLE_EXTENDED_FLAGS | ENABLE_WINDOW_INPUT | ENABLE_MOUSE_INPUT | ENABLE_PROCESSED_INPUT);
...
{
	// Updates screen
	WriteConsoleOutput(ConsoleOut, screen, { SCREENWIDTH_, SCREENHEIGHT_ }, { 0, 0 }, &m_rectWindow);
...
void Console::WriteChar(int x, int y, char character, byte color)
{
	if ((y < SCREENHEIGHT_) && (y >= 0) && (x < SCREENWIDTH_) && (x >= 0))
	{
		...
		screen[y * SCREENWIDTH_ + x].Attributes = color;
	}
}
